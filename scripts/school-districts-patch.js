...
async function schoolLocation(feature){
  const p=feature.properties;
  if(schoolGeocodeCache.has(p.id))return schoolGeocodeCache.get(p.id);
  const fallback=L.latLng(p.labelLat,p.labelLng);
  const query=`神奈川県${p.address} ${p.name}`;
  ...
  try{
    ...
    if(response.ok&&Array.isArray(c)&&c.length>=2){
      const ll=L.latLng(c[1],c[0]);schoolGeocodeCache.set(p.id,ll);return ll;
    }
  }catch{}
...
createRoute=async function(level){
  const feature=level==='elementary'?state.elementary:state.juniorHigh;
  if(!state.point||!feature||state.busy)return;
  ...
  try{
    ...
    route.points=thin(route.points);
    state.routeLine=L.polyline(route.points,{color:'#315f7b',weight:5,opacity:.9}).addTo(state.map);
    ...
    if(id===state.routeId){
      document.querySelector('#routeStatus').textContent=`徒歩ルートを表示できませんでした：${error.message}`;

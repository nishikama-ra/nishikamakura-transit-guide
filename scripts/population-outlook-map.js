...
function style(feature) { const value=feature.properties.values[String(state.year)][state.metric]||0; return { color:'rgba(255,255,255,.94)', weight:.55, fillColor:color(value), fillOpacity:value===0?0:.58 }; }
...
function addBaseMap(map){ map.createPane('baseMapPane'); const pane=map.getPane('baseMapPane'); pane.style.zIndex='200'; pane.style.background='#fff'; pane.style.filter='grayscale(1) contrast(1.15)'; L.tileLayer('https://cyberjapandata.gsi.go.jp/xyz/pale/{z}/{x}/{y}.png',{pane:'baseMapPane',maxZoom:18,opacity:.22,attribution:'<a href="https://maps.gsi.go.jp/development/ichiran.html" target="_blank" rel="noopener">地理院タイル</a>'}).addTo(map); }

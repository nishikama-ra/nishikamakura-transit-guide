...
function rgba(hex,alpha){ const value=hex.replace('#',''); const number=Number.parseInt(value,16); return `rgba(${number>>16},${number>>8&255},${number&255},${alpha})`; }
function style(feature) { const value=feature.properties.values[String(state.year)][state.metric]||0; return { color:'rgba(255,255,255,.72)', weight:.35, fillColor:color(value), fillOpacity:value===0?0:MESH_OPACITY }; }
...
function updateLegend(){ const root=document.querySelector('#populationLegend'); const b=BREAKS[state.metric]; const labels=['0（透明）',`${b[0]}～${b[1]-1}`,`${b[1]}～${b[2]-1}`,`${b[2]}～${b[3]-1}`,`${b[3]}～${b[4]-1}`,`${b[4]}以上`]; const swatches=['transparent',...POP_COLORS.map(value=>rgba(value,MESH_OPACITY))]; root.innerHTML=`<strong>${state.data.meta.metrics[state.metric]}（人／250mメッシュ）</strong>${labels.map((label,i)=>`<div class="mn-legend-row"><span class="mn-legend-swatch" style="background:${swatches[i]};${i===0?'border-style:dashed;':''}"></span>${label}</div>`).join('')}`; }
...
function addBaseMap(map){ map.createPane('baseMapPane'); const pane=map.getPane('baseMapPane'); pane.style.zIndex='200'; pane.style.background='#fff'; pane.style.filter='grayscale(1)'; L.tileLayer('https://cyberjapandata.gsi.go.jp/xyz/pale/{z}/{x}/{y}.png',{pane:'baseMapPane',maxZoom:18,opacity:1,attribution:'<a href="https://maps.gsi.go.jp/development/ichiran.html" target="_blank" rel="noopener">地理院タイル</a>'}).addTo(map); }

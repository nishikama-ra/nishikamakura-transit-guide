...
function style(feature) { const value=feature.properties.values[String(state.year)][state.metric]||0; return { color:'rgba(255,255,255,.72)', weight:.35, fillColor:color(value), fillOpacity:value===0?0:.16 }; }
...
function addBaseMap(map){ map.createPane('baseMapPane'); const pane=map.getPane('baseMapPane'); pane.style.zIndex='200'; pane.style.background='#fff'; pane.style.filter='grayscale(1) brightness(1.08) contrast(1.2)'; L.tileLayer('https://cyberjapandata.gsi.go.jp/xyz/std/{z}/{x}/{y}.png',{pane:'baseMapPane',maxZoom:18,opacity:1,attribution:'<a href="https://maps.gsi.go.jp/development/ichiran.html" target="_blank" rel="noopener">地理院タイル</a>'}).addTo(map); }

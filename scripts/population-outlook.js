const POP_COLORS = ['#d8e8f0','#acd0df','#73b2c9','#3e8eae','#17678f'];
...
function style(feature) { const value=feature.properties.values[String(state.year)][state.metric]||0; return { color:'rgba(255,255,255,.55)', weight:.35, fillColor:color(value), fillOpacity:value===0?0:.5 }; }
...
function setMapAspect(bounds){ const west=bounds.getWest(), east=bounds.getEast(), south=bounds.getSouth(), north=bounds.getNorth(); const mid=(south+north)/2*Math.PI/180; const width=Math.max(.001,(east-west)*Math.cos(mid)); const height=Math.max(.001,north-south); const ratio=Math.max(.72,Math.min(1.65,width/height)); document.querySelector('#populationMap').style.aspectRatio=String(ratio); document.querySelector('#populationMap').style.height='auto'; document.querySelector('#populationMap').style.minHeight='420px'; }
...
function makeTargetArea(topologies){ const selected=topologies.flatMap(topology=>topologyFeatures(topology)).filter(feature=>TARGET_TOWN_CODES.has(townCode(feature))); if(selected.length!==TARGET_TOWN_CODES.size) throw new Error(`対象区域の境界データが不足しています（${selected.length}/${TARGET_TOWN_CODES.size}件）`); return turf.union(turf.featureCollection(selected)); }
...
async function init(){ const [populationData,kamakuraTopology,fujisawaTopology]=await Promise.all([fetchJson('content/population-outlook.geojson','人口データ'),fetchJson(BOUNDARY_URLS.kamakura,'鎌倉市の区域境界'),fetchJson(BOUNDARY_URLS.fujisawa,'藤沢市の区域境界')]); const targetArea=makeTargetArea([kamakuraTopology,fujisawaTopology]); state.data=filterPopulationData(populationData,targetArea); state.map=L.map('populationMap',{scrollWheelZoom:true}).setView([35.323,139.505],13); L.tileLayer('https://cyberjapandata.gsi.go.jp/xyz/std/{z}/{x}/{y}.png',{maxZoom:18,attribution:'<a href="https://maps.gsi.go.jp/development/ichiran.html" target="_blank" rel="noopener">地理院タイル</a>'}).addTo(state.map); state.layer=L.geoJSON(state.data,{style,onEachFeature:(f,l)=>l.bindPopup(popup(f),{maxWidth:280})}).addTo(state.map); const bounds=state.layer.getBounds(); setMapAspect(bounds); setTimeout(()=>{state.map.invalidateSize(); state.map.fitBounds(bounds,{padding:[8,8]});},0); const range=document.querySelector('#yearRange'); range.min=0; range.max=state.data.meta.years.length-1; range.value=0; range.addEventListener('input',()=>{state.year=state.data.meta.years[Number(range.value)];render();}); document.querySelectorAll('[data-metric]').forEach(button=>button.addEventListener('click',()=>{state.metric=button.dataset.metric;document.querySelectorAll('[data-metric]').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));render();})); const legend=L.control({position:'bottomright'}); legend.onAdd=()=>{const div=L.DomUtil.create('div','mn-legend');div.id='populationLegend';return div;}; legend.addTo(state.map); render(); }

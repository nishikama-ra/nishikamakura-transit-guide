...
function featureToInstitution(feature) {
  ...
  return {
    id: `${schoolCode || 'unknown'}-${campusCode}`,
    schoolCode,
    campusCode,
    sourceName,
    displayName: DISPLAY_NAMES[sourceName] ?? sourceName,
    address: fullAddress(sourceAddress),
    municipality,
    ...
    ownership,
    officialUrl: OFFICIAL_URLS[sourceName] ?? null
  };
}
...
function compactOutput(output) {
  const ownershipCodes = { municipal: 'm', prefectural: 'p', national: 'n', private: 'r' };
  const urls = [];
  const urlIndexes = new Map();
  const urlIndex = value => {
    if (!value) return -1;
    if (!urlIndexes.has(value)) {
      urlIndexes.set(value, urls.length);
      urls.push(value);
    }
    return urlIndexes.get(value);
  };
  return {
    meta: { ...output.meta, schemaVersion: 2 },
    urls,
    campuses: output.campuses.map(campus => [
      campus.name,
      campus.address,
      campus.lat,
      campus.lng,
      campus.listings.map(listing => [
        listing.name,
        ownershipCodes[listing.ownerships[0]],
        [...new Set(listing.formalTypes.map(item => item.code))].join(','),
        urlIndex(listing.officialUrl),
        listing.institutionIds[0]
      ])
    ])
  };
}
...
const compact = compactOutput(buildOutput(source.features, path.basename(sourcePath), sourceSha256));
const outputDirectory = path.dirname(outputPath);
const outputStem = path.basename(outputPath, path.extname(outputPath));
const chunkSize = 50;
const chunkFiles = [];
fs.mkdirSync(outputDirectory, { recursive: true });
for (let offset = 0; offset < compact.campuses.length; offset += chunkSize) {
  const filename = `${outputStem}-${String(offset / chunkSize + 1).padStart(2, '0')}.json`;
  chunkFiles.push(filename);
  fs.writeFileSync(path.join(outputDirectory, filename), `${JSON.stringify(compact.campuses.slice(offset, offset + chunkSize))}\n`, 'utf8');
}
const output = {
  meta: { ...compact.meta, schemaVersion: 3 },
  urls: compact.urls,
  chunkFiles
};
fs.writeFileSync(outputPath, `${JSON.stringify(output)}\n`, 'utf8');
console.log(JSON.stringify({ output: outputPath, chunkFiles, institutionCount: output.meta.institutionCount, campusCount: output.meta.campusCount, countsByType: output.meta.countsByType, countsByMunicipality: output.meta.countsByMunicipality, sourceSha256 }, null, 2));
